...
def image_reduction(original_image):
    ...
    for i in range(IMAGE_REDUCTION_LEVELS, -1, -1):
        ...
        if local_maxima_coords:
            local_maxima_coords = scale_coordinates(local_maxima_coords)
...
def hough_method(edges_image, image_gradient_direction, current_iteration, max_radius=-1, local_maxima_coords=None):
    ...
    sorted_local_maxima_coords = get_local_maxima(accumulator, current_iteration)
...
def get_local_maxima(accumulator, current_iteration):
    ...
    return sorted_local_maxima_coords[:N_CIRCLES]
